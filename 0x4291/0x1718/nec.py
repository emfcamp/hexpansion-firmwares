...
class NEC_ABC(IR_RX):
    ...
    def decode(self, _):
        try:
            ...
            # The leader mark (times[1] - times[0]) is not checked. With the
            # busy-poll capture, times[0] is the capture start, which under host
            # load can land partway through the 9ms mark, making its measured
            # width unreliable. The leader space below, the edge count, and the
            # 32-bit checksum still fully validate the frame. self._leader is
            # retained only to document the nominal mark.
